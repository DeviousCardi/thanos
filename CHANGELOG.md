...
- [#9041](https://github.com/thanos-io/thanos/pull/9041): Query: Fix penalty deduplication injecting a spurious counter adjustment when the earliest sample of a query window belongs to the second replica, which flattened counters at the window start and made `rate()` under-report (most visible at query-range split boundaries).
